...
def update_plots(player):
    ...
    fig1.update_layout(title='Shot Speed Consistency', xaxis_title='Season Progress', yaxis_title='Speed of Shots', font=dict(size=18))

    # Homerun Pie Chart
    homerun_counts = temp.groupby('Season')['HomerunsOfSeasonSoFar'].max().to_dict()
    fig2 = go.Figure(data=[go.Pie(labels=list(homerun_counts.keys()), values=list(homerun_counts.values()))])
    fig2.update_layout(title="Homerun Consistency Over the Seasons", font=dict(size=18))

    # Hit Distance Histogram
    fig3 = px.histogram(temp, x='HitDistance', nbins=20, title='Distance Travelled by the Ball')
    fig3.update_layout(xaxis_title='Distance (in m)', yaxis_title='Frequency', font=dict(size=18))
    ...
    fig5.update_layout(title=dict(text=f'Exit Velocity Comparison with Best Player', font=dict(size=18)), xaxis_title='Season Progress', yaxis_title='Speed of Shots', legend=dict(font=dict(size=18)))
    ...
    fig6.update_layout(grid={'rows': 1, 'columns': 3}, height=400, width=1775, font=dict(size=18))
    ...
    fig7.update_layout(xaxis_title='Player Name', yaxis_title='Maximum Hit Distance', font=dict(size=18))
